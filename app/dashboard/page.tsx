    ...
    <div style={{ padding: '20px 24px', maxWidth: 960, margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10, marginBottom: 14 }}>
        {[
          ...
          { label: 'Missing items',  val: stats?.missing_count,        sub: 'Needs attention', subColor: '#EF4444', numColor: '#EF4444', iconColor: '#EF4444', iconBg: 'rgba(239,68,68,0.07)',    href: '/alerts',  iconName: 'ti-alert-triangle' },
          ...
          const inner = (
            <div className="card" style={{ padding: '14px 16px', position: 'relative', overflow: 'hidden', cursor: c.href ? 'pointer' : 'default' }}>
              {/* Subtle corner bubble */}
              <div style={{ position: 'absolute', width: 60, height: 60, borderRadius: '50%', background: c.iconBg, top: -15, right: -15, pointerEvents: 'none' }} />
              <i className={`ti ${c.iconName}`} style={{ fontSize: 16, color: c.iconColor, display: 'block', marginBottom: 10, position: 'relative' }} aria-hidden="true" />
              <div style={{ fontSize: 26, fontWeight: 500, letterSpacing: '-0.8px', color: c.numColor, lineHeight: 1, position: 'relative' }}>
                {loading ? '–' : c.val ?? 0}
              </div>
              <div style={{ fontSize: 11, color: '#9CA3AF', marginTop: 5 }}>{c.label}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10, marginBottom: 16 }}>
        {[
          ...
          <Link key={q.href} href={q.href}>
            <div className="card" style={{ padding: '11px 14px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 10, transition: 'border-color 0.12s' }}
