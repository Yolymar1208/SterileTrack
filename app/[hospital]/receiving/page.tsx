...
export default function ReceivingPage() {
  ...
  async function lookupItemWithDispense(code: string, dispense: DispensedItem | null) {
    ...
    // Preserve checked state and receivedQty from current contents if same item
    const prevContents = contents
    setContents((cList || []).map(c => {
      const prev = prevContents.find(p => p.id === c.id)
      return {
        ...c,
        checked: prev ? prev.checked : false,
        receivedQty: c.quantity, // always use DB quantity (already persisted)
      }
    }))
    setSearching(false)
  }
